...
public class FileChecksumHandler implements AsyncServerRequestHandler<Message<HttpRequest, byte[]>> {
    private FileServer fileServer;

    public FileChecksumHandler(FileServer fileServer) {
        this.fileServer = fileServer;
    }

    @Override
    public AsyncRequestConsumer<Message<HttpRequest, byte[]>>
            prepare(HttpRequest request, EntityDetails entityDetails, HttpContext context)
            throws HttpException {
        return new BasicRequestConsumer<>(new BasicAsyncEntityConsumer());
    }

    @Override
    public void handle(Message<HttpRequest, byte[]> message, ResponseTrigger responseTrigger, HttpContext context)
        ...
        if (!HttpHelper.validateNonEmptyParams(
                params, new String[] { "resource", "token", "path", "chunkSize" }, responseTrigger, context)) {
            return;
        }
        ...
        System.out.println("Request: " + fileServer.getUserByToken(params.get("token")).getLogin() + ": " + resourceName
                + " - " + params.get("path") + ", chunkSize: " + params.get("chunkSize") + "... ");
