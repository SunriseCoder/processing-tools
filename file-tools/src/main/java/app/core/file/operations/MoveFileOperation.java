...
public class MoveFileOperation implements FileOperation {
    private Path source;
    ...
    public MoveFileOperation(Path source, Path destination) throws IOException {
        this.source = source;
        this.destination = destination;

        Path sourceRoot = source.getRoot();
        Path destinationRoot = destination.getRoot();
        isMoveWithinSameDisk = sourceRoot.equals(destinationRoot);
    ...
    public void perform() throws IOException {
        ...
        if (isMoveWithinSameDisk) {
            ...
            PathUtils.moveFileWithReplacement(source, destination);
        } else {
            Path temporaryFile = Files.createTempFile(temporaryFolder, "move-file-tmp-", ".tmp");
            PathUtils.copyFile(source, temporaryFile, copyChunkSize, progressPrinter);

            if (lastModifiedTime != null) {
                // Set lastModifiedTime attribute to the temporary file after the copy is done
    ...
    public String toString() {
        ...
        return FormattingUtils.alignLongStringsByRightSide(3,
                "MOVE_FILE", source.toString(), fileSizeString,
